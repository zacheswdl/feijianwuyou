...
const AuthorizationForm = ({ open, record, mode, onCancel, onSave }: AuthorizationFormProps) => {
  ...
  useEffect(() => {
    if (open && !initialized) {
      ...
      if (record) {
        ...
        form.setFieldsValue({
          ...record,
          authorizeDate: record.authorizeDate && dayjs(record.authorizeDate).isValid() 
            ? dayjs(record.authorizeDate) 
            : undefined,
        });
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      const formattedValues = {
        ...values,
        authorizeDate: values.authorizeDate?.format('YYYY-MM-DD'),
        // 保存文件信息
  ...
  const getTitle = () => {
    switch (mode) {
      case 'add': return '新增授权文件';
      case 'edit': return '编辑授权文件';
      case 'view': return '查看授权文件';
      default: return '授权文件详情';
    }
  };
      ...
      >
        ...
        >
          <Form.Item
            label="授权文件编号"
            name="documentNo"
            rules={[{ required: true, message: '请输入授权文件编号' }]}
          >
            <Input placeholder="如：RAAYLS-SQ【2023001】号" />
          </Form.Item>
          ...
          >
            <TextArea 
              rows={3} 
              placeholder="请输入这份授权文件内所有人员的姓名，如：张三、李四、王五"
            />
          </Form.Item>
